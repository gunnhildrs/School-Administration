...
public class home extends JFrame {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            JPanel photoContainer = new JPanel() {
                ...
                protected void paintComponent(Graphics g) {
                    ...
                    try {
                        ImageIcon photoIcon = new ImageIcon(home.class.getResource(photoPaths[index]));
                        g2.drawImage(photoIcon.getImage(), 0, 0, width, height, this);
                    } catch (Exception e) {
                        g2.setColor(Color.LIGHT_GRAY);
                        g2.fill(photoRect);
                        g2.setColor(Color.DARK_GRAY);
                        g2.drawString(photoLabels[index], width/4, height/2);
                    }
    ...
    private void openMapel(){
        try {
            // Create and show the DataSiswaa window
            SwingUtilities.invokeLater(() -> {
                Mapel dataSiswaWindow = new Mapel();
                dataSiswaWindow.setVisible(true);
            }); 
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, 
