...
class CustomHeaderRenderer extends javax.swing.table.DefaultTableCellRenderer {
    ...
    try {
        ...
        // Path ke file jasper yang benar
        String reportPath = "src/view/Suket_lulus.jasper";

        // Cek apakah file jasper ada
        File jasperFile = new File(reportPath);
        if (!jasperFile.exists()) {
            JOptionPane.showMessageDialog(this, 
                "File laporan tidak ditemukan: " + reportPath + 
                "\nPastikan file Suket_lulus.jasper ada di folder src/view/", 
                "Error", 
                JOptionPane.ERROR_MESSAGE);
            return;
        }
