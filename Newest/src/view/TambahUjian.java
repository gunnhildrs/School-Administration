...
public class TambahUjian extends javax.swing.JFrame {
...
private String getIdFromComboBox(JComboBox<String> comboBox) {
    ...
    private void jButton11ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        // TODO add your handling code here:
        TambahUjian homeForm = new TambahUjian();
    ...
    // Build time strings
    String waktuMulai = jComboBox7.getSelectedItem() + ":" + jComboBox6.getSelectedItem();
    String waktuSelesai = jComboBox5.getSelectedItem() + ":" + jComboBox2.getSelectedItem();
    ...
    Connection conn = getConnection();
    if (conn != null) {
        // Fixed: Removed durasi_menit from INSERT since it's a generated column
        String query = "INSERT INTO ujian (id_ujian, nama_ujian, id_pelajaran, " +
                     "id_kelas, id_ruang, tanggal_ujian, waktu_mulai, waktu_selesai, " +
                     "jumlah_peserta) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        
        PreparedStatement pst = conn.prepareStatement(query);
        pst.setString(1, jTextField2.getText());  // id_ujian
        pst.setString(2, jTextField4.getText());  // nama_ujian
        pst.setString(3, idPelajaran);            // id_pelajaran
        pst.setString(4, idKelas);                // id_kelas
        pst.setString(5, idRuang);                // id_ruang
        pst.setString(6, tanggalUjian);           // tanggal_ujian
        pst.setString(7, waktuMulai);             // waktu_mulai
        pst.setString(8, waktuSelesai);           // waktu_selesai
        pst.setString(9, jTextField1.getText());  // jumlah_peserta
        
        int result = pst.executeUpdate();
        
        if (result > 0) {
            JOptionPane.showMessageDialog(this, "Data ujian berhasil ditambahkan!");
            kosong(); // Clear form after successful save
        } else {
            JOptionPane.showMessageDialog(this, "Gagal menambahkan data ujian!");
        }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(TambahUjian.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new TambahUjian().setVisible(true);
            }
        });
    }
